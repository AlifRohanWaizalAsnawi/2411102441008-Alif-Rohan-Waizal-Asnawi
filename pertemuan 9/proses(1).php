...
<div class="container">
    ...
    <?php
    // Cek apakah data sudah dikirimkan melalui metode GET
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        
        // MENGAMBIL DATA MENGGUNAKAN $_GET[]
        $nama_lengkap = isset($_GET['nama']) ? htmlspecialchars($_GET['nama']) : 'XXX';
        $tempat_lahir = isset($_GET['tempat_lahir']) ? htmlspecialchars($_GET['tempat_lahir']) : 'XXX';
        $tgl_lahir_tgl = isset($_GET['tgl_lahir_tgl']) ? htmlspecialchars($_GET['tgl_lahir_tgl']) : 'TGL';
        $tgl_lahir_bln = isset($_GET['tgl_lahir_bln']) ? htmlspecialchars($_GET['tgl_lahir_bln']) : 'BLN';
        $tgl_lahir_thn = isset($_GET['tgl_lahir_thn']) ? htmlspecialchars($_GET['tgl_lahir_thn']) : 'THN';
        $alamat_rumah = isset($_GET['alamat']) ? nl2br(htmlspecialchars($_GET['alamat'])) : 'XXX';
        $jenis_kelamin = isset($_GET['jenis_kelamin']) ? htmlspecialchars($_GET['jenis_kelamin']) : 'XXX';
        $asal_sekolah = isset($_GET['asal_sekolah']) ? htmlspecialchars($_GET['asal_sekolah']) : 'XXX';
        $nilai_uan = isset($_GET['nilai_uan']) ? htmlspecialchars($_GET['nilai_uan']) : 'XXX';
